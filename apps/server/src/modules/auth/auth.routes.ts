...
import { jwtSignPlugin } from '../../shared/plugins/auth.plugin'
import { errorSchema } from '../../shared/types/error.type'

export const auth = new Elysia({ prefix: '/auth', tags: ['auth'] }).use(jwtSignPlugin).post(
  '/login',
  async ({ jwtSign, body, status }) => {
    ...
    const token = await jwtSign.sign({ userId: result.data.id, username: result.data.username })
    return { token }
  },
  {
